...
public class TopNavigationBar extends VBox {
    ...
    private void buildTabButtons() {
        ...
        Button marketBtn = createTabButton("MARKET");
        ...
        HBox.setHgrow(marketBtn, Priority.ALWAYS);
        ...
        tabsContainer.getChildren().addAll(craftBtn, refineBtn, specsBtn, marketBtn, optionsBtn);
    ...
    public void setOnTabClicked(String text, Runnable action) {
        for (javafx.scene.Node navigationNode : getChildren()) {
            if (!(navigationNode instanceof HBox tabs)) {
                continue;
            }

            for (javafx.scene.Node tabNode : tabs.getChildren()) {
                if (!(tabNode instanceof Button button)) {
                    continue;
                }
