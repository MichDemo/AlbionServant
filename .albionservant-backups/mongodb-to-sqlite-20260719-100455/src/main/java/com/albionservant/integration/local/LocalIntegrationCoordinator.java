...
import com.albionservant.market.service.MarketMongoIndexInitializer;
...
public class LocalIntegrationCoordinator {
    ...
    private final MarketMongoIndexInitializer indexInitializer;
    ...
    public LocalIntegrationCoordinator(
            ...
            MarketMongoIndexInitializer indexInitializer,
    ...
    private void startInternal() {
        status.update(LocalIntegrationStatus.State.STARTING, "Starting MongoDB and NATS");

        try {
            endpointGuard.validate();
            processManager.startMongoIfNeeded();
            processManager.startNatsIfNeeded();
            indexInitializer.initialize();
            natsRuntime.start();
            processManager.startAodpIfNeeded();

            status.update(LocalIntegrationStatus.State.RUNNING, "Local market ingestion is running");
            log.info("AlbionServant local market integration is running");
        } catch (Exception e) {
