    ...
    public void ingest(AodpMarketUpload upload) {
        ...
        for (Map.Entry<PriceKey, PriceAccumulator> entry : prices.entrySet()) {
            ...
            Update update = new Update()
                    .setOnInsert("server", key.server)
                    .setOnInsert("itemId", key.itemId)
                    .setOnInsert("locationId", key.locationId)
                    .setOnInsert("qualityLevel", key.qualityLevel)
                    .setOnInsert("enchantmentLevel", key.enchantmentLevel)
                    .set("observedAt", observedAt);

            if (value.minSell != null) {
                update.set("minSellPrice", value.minSell);
                update.set("sellUpdatedAt", observedAt);
            }

            if (value.maxBuy != null) {
                update.set("maxBuyPrice", value.maxBuy);
                update.set("buyUpdatedAt", observedAt);
            }

            mongoTemplate.upsert(
                    Query.query(Criteria.where("_id").is(id)),
                    update,
                    MarketPriceDocument.class
            );
            updatedKeys.add(id);
        }
