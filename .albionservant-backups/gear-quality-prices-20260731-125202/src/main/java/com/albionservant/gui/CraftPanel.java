...
topSection.getChildren().addAll(left, center, right, reqColumn);
            ...
            whiteContent.getChildren().addAll(topSection, buildTable(itemName, reqPanel, quantity, buy1, buy2, buy3), bottom);
                ...
                for (int i = 0; i < resultValues.length; i++) {
                    if (i == 1) {
                        ...
                        Runnable updateCostCell = () -> {
                            ...
                            if (!hasEveryPrice
                                    || rowApiFields.isEmpty()) {
                                costCell.setText("\u2014");
                                return;
                            }

                            long requestedCrafts = 1L;

                            try {
                                requestedCrafts = Math.max(
                                        1L,
                                        Long.parseLong(
                                                quantityField
                                                        .getText()
                                ...
                                // Keep one craft for invalid quantity input.
                            }
