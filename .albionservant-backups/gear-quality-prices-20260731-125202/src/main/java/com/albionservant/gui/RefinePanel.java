...
public class RefinePanel extends VBox {
    ...
    private ComboBox<String> buyRawLocation;
    private ComboBox<String> sellRefinedLocation;
    ...
    private HBox buildConfigSection(MaterialFamily family) {
        ...
        buyRawLocation = cfgCombo(
                citiesArray(),
                "Fort Sterling"
        );
        sellRefinedLocation = cfgCombo(
                citiesArray(),
                "Fort Sterling"
        );

        ComboBox<String> refineTier = cfgCombo(
                new String[]{"T2", "T3", "T4", "T5", "T6", "T7", "T8"},
                "T5"
        );
        ComboBox<String> stackFrom = cfgCombo(
                new String[]{"T2", "T3", "T4", "T5", "T6", "T7", "T8"},
                "T2"
        );
        ComboBox<String> stackTo = cfgCombo(
                new String[]{"T2", "T3", "T4", "T5", "T6", "T7", "T8"},
                "T8"
        );
        ...
        right.getChildren().addAll(
                cfgLabel("Buy Raw At:"),
                buyRawLocation,
                cfgLabel("Sell Refined At:"),
                sellRefinedLocation,
                cfgLabel("Refine Tier:"),
                refineTier,
                cfgLabel("Stack From:"),
                stackFrom,
                cfgLabel("Stack To:"),
    ...
    private VBox buildRefineGrid(MaterialFamily family) {
        ...
        for (int ti = 0; ti < RefineData.TIERS.length; ti++) {
            ...
            for (int enc : enchants) {
                ...
                TextField previousApi = hasT2
                        ? naField()
                        : apiInput(bg);

                TextField previousManual = hasT2
                        ? naField()
                        : editInput(bg);

                TextField outputApi = apiInput(bg);
                TextField outputManual = editInput(bg);

                String rawItemId =
                        LocalMarketPriceService.withEnchant(
                                family.rawIconIds()[ti],
                                enc
                        );

                String outputItemId =
                        LocalMarketPriceService.withEnchant(
                                family.refinedIconIds()[ti],
                                enc
                        );
                ...
                grid.add(rawApi, 1, dataRow);
                grid.add(rawManual, 2, dataRow);
                grid.add(previousApi, 3, dataRow);
                grid.add(previousManual, 4, dataRow);
                grid.add(outputApi, 5, dataRow);
                grid.add(outputManual, 6, dataRow);

                Label demandLabel = resultLabel("\u2014", bg);
                Label profitLabel = resultLabel("\u2014", bg);
                Label spfLabel = resultLabel("\u2014", bg);
                Label gainLabel = resultLabel("\u2014", bg);
                Label costLabel = resultLabel("\u2014", bg);
                Label focusLabel = resultLabel("\u2014", bg);
                ...
                Runnable updateEconomy = () -> {
                    boolean hasRaw =
                            LocalMarketPriceService.hasEffectivePrice(
                                    rawApi,
                                    rawManual
                            );

                    boolean hasPrevious = hasT2
                            || LocalMarketPriceService.hasEffectivePrice(
                            previousApi,
                            previousManual
                    );

                    if (!hasRaw || !hasPrevious) {
                        costLabel.setText("\u2014");
                        profitLabel.setText("\u2014");
                        gainLabel.setText("\u2014");
                        return;
                    ...
                    double inputCost =
                            rawPrice
                                    * RefineData.rawPerOutput(tier)
                                    + previousPrice
                                    * RefineData
                                    .prevRefinedPerOutput(tier);
                    ...
                    double profit = outputPrice - inputCost;
                ...
                grid.add(demandLabel, 7, dataRow);
                grid.add(profitLabel, 8, dataRow);
                grid.add(spfLabel, 9, dataRow);
                grid.add(gainLabel, 10, dataRow);
                grid.add(costLabel, 11, dataRow);
                grid.add(focusLabel, 12, dataRow);
                dataRow++;
            } // end for enc
        } // end for tier
    ...
    private Label resultLabel(String text, String bg) {
        Label label = new Label(text);
        label.setStyle(resultCellStyle(bg));
